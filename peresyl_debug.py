...
# Флаг для отслеживания необходимости переслать следующее сообщение
forward_next_message = False

async def debug_all(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Ловим ВСЕ обновления для отладки"""
    logger.debug(f"=== ПОЛУЧЕНО ОБНОВЛЕНИЕ ===")
    logger.debug(f"Chat ID: {update.effective_chat.id if update.effective_chat else 'None'}")
    logger.debug(f"Chat type: {update.effective_chat.type if update.effective_chat else 'None'}")
    logger.debug(f"User: {update.effective_user.full_name if update.effective_user else 'None'}")
    logger.debug(f"Text: {update.effective_message.text if update.effective_message else 'None'}")
    logger.debug(f"SOURCE_GROUP_ID в коде: {SOURCE_GROUP_ID}")

async def open_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Команда /open - разрешает пересылку следующего сообщения (доступна всем в группе)"""
    global forward_next_message
    
    # Проверяем, что команда из нужной группы
    if str(update.effective_chat.id) != str(SOURCE_GROUP_ID):
        return
    
    forward_next_message = True
    await update.message.reply_text(
        "✅ Режим пересылки активирован!\n"
        "Следующее сообщение в группе будет переслано в канал."
    )
    logger.info(f"Команда /open выполнена пользователем {update.effective_user.id} в группе")

async def forward_to_channel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Пересылка сообщений из группы в канал с кнопками (только после /open)"""
    global forward_next_message
    ...
    try:
        ...
        # Если режим пересылки не активен - игнорируем сообщение
        if not forward_next_message:
            return
        ...
        # Сбрасываем флаг (пересылаем только одно сообщение)
        forward_next_message = False
...
def main():
    ...
    application.add_handler(MessageHandler(filters.ALL, debug_all), group=-1)  # DEBUG
